...
export default function ChatRoomScreen() {
  ...
  // Додає вхідне повідомлення, узгоджуючи оптимістичні (pending) копії та дублікати.
  ...
  // ── Завантаження історії + підключення сокета ────────────────────
  useEffect(() => {
    let cancelled = false;
    ...
    const onRoomJoined = (p: { roomId: number; history: ChatMessageDTO[] }) => {
      ...
      // Сервер — джерело правди, коли онлайн
      setMessages(
        p.history.map((m) => ({
          id: m.id,
          senderId: m.senderId,
  ...
  // ── Індикатор набору (throttle + debounce-off) ──────────────────
  ...
  const send = () => {
    ...
    if (socketLive && token) {
      const socket = connectSocket(token);
      // Оптимістичне повідомлення (тимчасовий відʼємний id), сервер замінить його через new_message
      ...
      // Fallback (демо/офлайн): локальне відлуння без сервера
      setMessages((prev) => [
        ...prev,
...
// ─────────────────────────────────────────────────────────────────────
//  Typing indicator (три пульсуючі крапки)
// ─────────────────────────────────────────────────────────────────────
