...
export default function AnimalDetailScreen() {
  ...
  if (loading) return <LoadingSkeleton height={photoHeight} />;
  ...
  if (isWideScreen) {
    return (
      <View style={{ flex: 1, backgroundColor: Colors.neutral[0], flexDirection: 'row' }}>
        {/* Left Column - Photo */}
        <View style={{ width: photoWidth, height: windowHeight }}>
          <PhotoCarousel photos={photos} width={photoWidth} height={windowHeight} />
        ...
        <View style={{ width: contentWidth, height: windowHeight, backgroundColor: Colors.neutral[0] }}>
          <ScrollView showsVerticalScrollIndicator={false} bounces contentContainerStyle={{ paddingBottom: 120 }}>
            <View style={{ paddingTop: Spacing[6], paddingHorizontal: Spacing[5] }}>
              <AnimalContentInner animal={animal} header={header} stats={stats} about={about} shelter={shelter} />
            </View>
          </ScrollView>

          {/* Fixed bottom action bar (pinned to right column) */}
          <Animated.View style={actions.anim}>
            ...
            >
              <View style={{ flexDirection: 'row', gap: Spacing[2], paddingHorizontal: Spacing[4], paddingTop: Spacing[3], paddingBottom: Spacing[2] }}>
                ...
                <ActionBtn label="Message" variant="primary" icon={<MessageCircle size={16} color={Colors.neutral[0]} strokeWidth={1.8} />} onPress={openChat} />
                <ActionBtn label="Book Visit" variant="outline" icon={<CalendarDays size={16} color={Colors.primary[500]} strokeWidth={1.8} />} onPress={openBooking} />
              </View>
            </SafeAreaView>
          </Animated.View>
        </View>

        <DonationSheet visible={donationVisible} onClose={() => setDonationVisible(false)} animalId={animal.id} animalName={animal.name} shelterId={animal.shelterId} />
      </View>
    );
  }
