...
function TabItem({ tab, isFocused, onPress, onLongPress }: TabItemProps) {
  ...
  const bounceAnim  = useRef(new Animated.Value(0)).current;   // translateY
  const glowAnim    = useRef(new Animated.Value(0)).current;   // pill opacity
  const labelAnim   = useRef(new Animated.Value(0)).current;   // label opacity+width
  ...
  useEffect(() => {
    if (isFocused) {
      ...
      Animated.spring(labelAnim, {
        toValue: 1, tension: 280, friction: 22, useNativeDriver: true,
      }).start();
  ...
  // Tap press-in feedback
  const handlePressIn = useCallback(() => {
    Animated.spring(scaleAnim, {
      toValue: 0.88, useNativeDriver: true, tension: 400, friction: 20,
    }).start();
  }, []);

  const handlePressOut = useCallback(() => {
    Animated.spring(scaleAnim, {
      toValue: isFocused ? 1 : 1, useNativeDriver: true, tension: 400, friction: 20,
    }).start();
  }, [isFocused]);
  ...
  // Glow pill scale
  const pillScale = glowAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0.6, 1],
  });
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.spring(entranceAnim, {
        toValue: 0, useNativeDriver: true, tension: 260, friction: 22,
      }),
  ...
  return (
    <Animated.View
      style={{
        ...
        opacity: opacityAnim,
        // Crisp white pill with warm shadow
