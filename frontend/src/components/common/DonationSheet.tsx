...
export interface DonationSheetProps {
  ...
  animalName: string;
  shelterId: number;
  /** If passed, sheet opens directly in GUARDIANSHIP mode */
    ...
    <Pressable onPress={onSelect} onPressIn={onIn} onPressOut={onOut} style={{ flex: 1 }}>
      ...
      >
        {popular && !selected && (
          ...
          >
            <Text style={{ fontSize: 9, fontWeight: '700', color: Colors.primary[600] }}>
              POPULAR
            </Text>
          </View>
        )}
...
function ModeTab({
  ...
  return (
    <Pressable onPress={onPress} onPressIn={onIn} onPressOut={onOut} style={{ flex: 1 }}>
      ...
      >
        {icon}
        <Text style={{ fontSize: 13, fontWeight: '700', color: active ? Colors.neutral[0] : Colors.neutral[500] }}>
          {label}
        </Text>
        <Text style={{ fontSize: 10, color: active ? 'rgba(255,255,255,0.75)' : Colors.neutral[400], textAlign: 'center' }}>
          {description}
        </Text>
      </Animated.View>
    </Pressable>
...
function SuccessView({ mode, amount, onClose }: { mode: DonationMode; amount: number; onClose: () => void }) {
  ...
  return (
    <Animated.View
      style={{ opacity, transform: [{ scale }], alignItems: 'center', paddingVertical: Spacing[8], gap: Spacing[4] }}
    >
...
export function DonationSheet({
  ...
  const close = useCallback(() => {
    Animated.parallel([
      Animated.timing(slideY, { toValue: 600, duration: Duration.slow, useNativeDriver: true, easing: Easing.out(Easing.cubic) }),
  ...
  // reset amount when switching mode
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === 'ONE_TIME') {
        ...
        const res = await donationService.createGuardianship({
          animalId,
      ...
      // open WebView after short delay to show success state
      setTimeout(() => {
        close();
  ...
  return (
    <Modal transparent animationType="none" visible={visible} onRequestClose={close} statusBarTranslucent>
      ...
      >
        {/* ── Backdrop ──────────────────────────────────── */}
        <Animated.View
          style={{ flex: 1, backgroundColor: Colors.overlay.dark, opacity: backdropOpacity }}
        >
            ...
            {success ? (
              ...
              <>
                ...
                >
                  ...
                  <ModeTab
                    label="Monthly"
                    description="Become guardian"
                    active={mode === 'GUARDIANSHIP'}
                    onPress={() => setMode('GUARDIANSHIP')}
                ...
                {mode === 'GUARDIANSHIP' && (
                  ...
                  >
                    <Sparkles size={16} color={Colors.primary[500]} strokeWidth={1.8} style={{ marginTop: 1 }} />
                ...
                >
                  <Text style={{ fontSize: 18, fontWeight: '700', color: Colors.neutral[400], marginRight: Spacing[1] }}>
                    ₴
                  </Text>
                  <TextInput
                    value={customAmt}
                ...
                <Pressable
                  ...
                  style={({ pressed }) => ({
                    backgroundColor: (!finalAmount)
                      ? Colors.neutral[200]
                      : pressed ? Colors.primary[600] : Colors.primary[500],
                    borderRadius: Radius.lg,
                    height: 56,
                    alignItems: 'center',
                    justifyContent: 'center',
