...
// Спільний екран списку чатів для адоптер- і шелтер-оболонок.
// `roomBasePath` визначає, у яку групу відкривати кімнату
// (напр. '/(app)/chat' або '/(shelter)/chat'), щоб не вистрибувати зі своєї оболонки.
...
export function ChatListView({ roomPathname }: { roomPathname: RoomPathname }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // лишаємо порожній список — нижче покажеться EmptyState
    } finally {
      setLoading(false);
    }
  }, []);
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyWrap}>
              <EmptyState
                title="Поки що тихо"
                subtitle={'Повідомлення зʼявляться, коли\nз вами звʼяжуться щодо тварини'}
              />
            </View>
          }
...
function ChatRow({ room, roomPathname }: { room: ChatRoom; roomPathname: RoomPathname }) {
  ...
  return (
    <TouchableOpacity
      onPress={() => router.push({ pathname: roomPathname, params: { id: room.id } })}
      activeOpacity={0.7}
      style={styles.row}
    >
      <Avatar uri={room.animal.primaryPhotoUrl} emoji={SPECIES_EMOJI[room.animal.species]} size={56} />
...
const styles = StyleSheet.create({
  ...
  shelterName: { flex: 1, fontSize: FontSize.md, fontWeight: FontWeight.bold, color: Colors.neutral[900] },
  ...
  animalTag: { fontSize: FontSize.xs, color: Colors.primary[500], fontWeight: FontWeight.medium, marginTop: 1 },
